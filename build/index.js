...
class Search {
  ...
  getResults() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default().getJSON(universityData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.val(), results => {
      this.resultsDiv.html(`
        <div class="row">
        ${results.generalInfo.length ? '<div class="one-third">' : ''}
          ${results.generalInfo.length ? '<h2 class="search-overlay__section-title">General Information</h2>' : ''}
            ${results.generalInfo.length ? '<ul class="link-list min-list">' : "<p></p>"}
